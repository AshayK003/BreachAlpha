...
export function BatchResults({ data }) {
  ...
  const SortIcon = ({ active }) =>
    active ? (
      <svg className="w-3 h-3 inline ml-0.5 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={sortDir === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7'} />
      </svg>
    ) : (
      <svg className="w-3 h-3 inline ml-0.5 text-muted-foreground/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const Th = ({ sort, children, className }) => (
    <th
      className={`text-left py-2 px-3 text-muted-foreground font-medium select-none ${
        sort ? 'cursor-pointer hover:text-foreground transition-colors' : ''
      } ${className || ''}`}
  ...
  return (
    <div className="card p-6 bg-card border border-border rounded-lg animate-in fade-in">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-xs font-semibold tracking-wider uppercase font-mono text-muted-foreground">
          Batch Results
        </h4>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[
          ...
          <div key={s.label} className="p-3 text-center bg-surface border border-border rounded-lg">
            ...
            <div className="text-[0.625rem] tracking-wider uppercase text-muted-foreground">{s.label}</div>
          </div>
        ))}
      </div>
            ...
            <tbody>
              {sorted.map((r, i) => (
                <tr
                  ...
                  className="border-b border-border/30 hover:bg-surface/50 cursor-pointer transition-colors"
                >
                  <td className="py-2 px-3 text-foreground font-medium">{r.company}</td>
                  <td className="py-2 px-3 text-muted-foreground">{r.ticker}</td>
                  <td className="py-2 px-3 text-muted-foreground whitespace-nowrap">{r.breach_date}</td>
                  <td
